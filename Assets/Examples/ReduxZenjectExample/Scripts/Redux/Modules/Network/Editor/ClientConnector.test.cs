...
namespace Reduxity.Example.Zenject.ClientConnector
{
	...
	public class Test : ZenjectUnitTestFixture {
		...
		private ClientState mockNetworkState_;
		...
		public void Setup() {
			...
			// set default state
			mockNetworkState_ = new ClientState {};
		...
		public void Should_set_isConnecting_and_isConnected_when_starting_request() {
			...
			Assert.IsTrue(result.isDisconnected);
			Assert.IsFalse(result.isDisconnectionFailed);
		...
		public void Should_set_Network_state_when_request_succeeds() {
			...
			Assert.IsFalse(result.isConnectionFailed);
			Assert.IsFalse(result.isDisconnecting);
			Assert.IsFalse(result.isDisconnected);
			Assert.IsFalse(result.isDisconnectionFailed);
			Assert.IsInstanceOf<string>(result.feedbackText);
			Assert.IsNotEmpty(result.feedbackText);
        }
		...
		[Test]
		public void Should_set_isDisconnected_when_disconnecting_succeeds() {
            // arrange
		...
		[Test]
		public void Should_set_isDisconnectionFailed_when_disconnecting_fails() {
            // arrange
			var mockNetworkAction = new Action.DisconnectFailure {
			};
			...
			Assert.IsFalse(result.isDisconnecting);
			Assert.IsFalse(result.isDisconnected);
			Assert.IsTrue(result.isDisconnectionFailed);
			Assert.IsInstanceOf<string>(result.feedbackText);
			Assert.IsNotEmpty(result.feedbackText);
        }
    }
